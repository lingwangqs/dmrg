...
//------------------------------------------------------------------------------
  ...
  for(i=0;i<=ns/2-2;i++){
    ...
    if(comm_rank==1)	cout<<"rank1 i="<<i<<endl;
    ...
    if(0&&(i+2)*2*physpn>=totspin&&i>=1||i==ns/2-2){
      if(comm_rank==0){
...
#pragma omp parallel for default(shared) private(j,k,m,m0,myrank,fac,htmp,tconj) schedule(dynamic,1)
  for(m0=0;m0<m5;m0++){
	...
	if(i==plqpos[j][1])
      ...
      cout<<"prepare_site_operator_from_left"<<"\tplqopr["<<i<<"]["<<m<<"].get_nbond()="<<plqopr[i][m].get_nbond()<<endl;
      //exit(0);
    }
  }
  save_memory(i-1,0);
...
#pragma omp parallel for default(shared) private(m,m0,j,k,myrank,fac,htmp,tconj) schedule(dynamic,1)
  for(m0=0;m0<m5;m0++){
	...
	if(i==plqpos[j][2])
      ...
      cout<<"prepare_site_operator_from_right"<<"\tplqopr["<<i<<"]["<<m<<"].get_nbond()="<<plqopr[i][m].get_nbond()<<endl;
      //exit(0);
    }
  }
  save_memory(i+1,1);
